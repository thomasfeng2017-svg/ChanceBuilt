# Agent setup for `autoshop`

This is a deepsec scanning workspace. Project `autoshop` was just registered
(target: `..`). Setup is incomplete — `data/autoshop/INFO.md`
...
2. **Fill in `data/autoshop/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id autoshop
pnpm deepsec process --project-id autoshop
```
